...
TP * retira_k(TP **vet, int n, int k){
  ...
  for(int i=0; i<n; i++){
    //tinha feito malloc que estava sobreescrevendo aonde não queria
    aux[i] = vet[i];
  }
  ...
  for(int i=0; i<n; i++){
    //acho que na prova não coloquei o k-1 como tinha feito acima (k-ésimo elemento, mas o índice do vetor começa em 0)
